...
/**
 * Lista de séries com ordenação por algoritmo mergesort.
 *
...
public class ListaSeries {
    ...
    private void intercalar(int esq, int meio, int dir) {
        ...
        // definir tamanho dos subarranjos
        n1 = meio - esq + 1;
        n2 = dir - meio;
        ...
        for (i = 0; i < n1; i++) {
            a1[i] = lista[esq + i].clone();
        }

        // inicializar segundo subarranjo
        for (j = 0; j < n2; j++) {
            a2[j] = lista[meio + j + 1].clone();
        }
    ...
    public void logFile(double t) throws Exception {
        FileWriter fw = new FileWriter("742626_mergesort.txt");
