...
int getMenor(int i, int j) {
    if (i == j) return i;
    int menor = getMenor(i + 1, j);

    char *s1 = trim(array[i].pais);
    char *s2 = trim(array[menor].pais);
    char *t1 = array[i].nome;
    char *t2 = array[menor].nome;

    cmpCount++;
    if (strcmp(s1, s2) > 0 || (strcmp(s1, s2) == 0 && strcmp(t1, t2) > 0)) {
        return menor;
    } else {
        return i;
    }    
...
void selecao(int i) {
    ...
    int menor = getMenor(i, n - 1);
    if (menor != i) swap(&array[i], &array[menor]);

    selecao(i + 1);
}

void logFile(double runtime) {
    FILE *arquivo = fopen("742626_selecaoRecursiva.txt", "w");
