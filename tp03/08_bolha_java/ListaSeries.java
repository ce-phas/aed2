...
public class ListaSeries {
    ...
    private int cmpNome(Series s1, Series s2) {
    ...
    private void sort() {
        for (int i = (size - 1); i > 0; i--) {
            for (int j = 0; j < i; j++) {
                ...
                int y = lista[j + 1].getNumeroTemporadas();

                cmpCount++;
                if (x > y || (x == y && cmpNome(lista[j], lista[j+1]) > 0)) {
                    swap(j, j+1);
                    swpCount++;
                }
            }
    ...
    public void logFile(double t) throws Exception {
        FileWriter fw = new FileWriter("742626_bolha.txt");
